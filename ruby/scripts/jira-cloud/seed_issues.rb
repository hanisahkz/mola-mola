...
totalRequests = 10 # Customize accordingly
hydra = Typhoeus::Hydra.new
requests = totalRequests.times.map {
    data = {
      "fields": {
        "project": {
          "id": "#{%w(10002).sample}" # To randomize targeted projects for issue creation, use: "#{%w(10005 10100 10006).sample}
        },
        "summary": "#{SecureRandom.hex(10)} test cloud",
        "issuetype": {
          "id": "#{%w(10009 10010 10011).sample}" # To randomize issue type, use: "id": "#{%w(10000 10001 10002 10004 10100).sample}"
        }
      }
    }
